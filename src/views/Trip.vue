        ...
        <div v-else>
          ...
          >
            ...
            <ion-card-header>
              ...
              <ion-card-subtitle
                >{{ voyage.description.slice(0, 50) }}...</ion-card-subtitle
              >
            </ion-card-header>

            <ion-card-content
              v-if="
                !editingMode || (editingMode && editedVoyage.id !== voyage.id)
              "
            >
              <ion-button fill="clear" @click.stop="editCard(voyage)"
                >Modifier</ion-button
              >
              <ion-button fill="clear" @click.stop="enSavoirPlus(voyage)"
                >Voir Détails</ion-button
              >
              <ion-button
                @click="confirmDelete(voyage)"
                expand="full"
                color="danger"
            ...
            >
              ...
              <p v-if="isFieldsEmpty()" class="error-message">
                Merci de renseigner les informations manquantes.
              </p>

              <ion-button @click.stop="saveChanges(voyage)"
  ...
  try {
    ...
    if (userID) {
      ...
      const response = await axios.get(
        `https://my-travel-log-cfax.onrender.com/api/trips?user=${userId.value}`
      );
...
const enSavoirPlus = (voyage) => {
  ...
  router.push({
    name: "TripDetails",
    params: { id: voyage.id, tripId: voyage.tripId },
  });
...
const saveChanges = async (originalVoyage) => {
  console.log(originalVoyage.id);
  console.log(editedVoyage.value.description);
  try {
    const token = AuthService.getToken();
    const response = await axios.patch(`https://my-travel-log-cfax.onrender.com/api/trips/${originalVoyage.id}`, {
      'title': editedVoyage.value.id,
      'description': editedVoyage.value.description,
    }, {
      headers: {
...
//         headers: {
//           'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3M…4MDR9.peJ2uVrTkqstmR6nKJv1gfGjIISPCNixvC-1Os0evE0',
//           'Content-Type': 'application/json',
//         },
//       });
...
const isFieldsEmpty = () => {
  return (
    editedVoyage.value.title === "" || editedVoyage.value.description === ""
  );
};
...
const confirmDelete = async (voyage) => {
  ...
  if (token) {
    if (confirm("Tu veux vraiment supprimer le voyage ?")) {
      console.log("oui je supp");
      try {
        console.log("oui je try");
        const response = await axios.delete(
          `https://my-travel-log-cfax.onrender.com/api/trips/${voyage.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (userID) {
          ...
          voyage.value = updateResponse.data;
        }

        console.log(
          "Réponse de la requête Axios (suppression) : ",
          response.data
        );
      } catch (eror) {
        if (error.response) {
          if (error.response.status === 404) {
            console.error(
              ...
              "Aucun voyage trouvé avec l'ID spécifié."
            );
            ...
            console.error(
              "Erreur lors de la suppression du voyage :",
              "Action interdite. Vous n'avez pas les autorisations nécessaires."
            );
          } else {
            console.error(
              "Erreur lors de la suppression du voyage :",
